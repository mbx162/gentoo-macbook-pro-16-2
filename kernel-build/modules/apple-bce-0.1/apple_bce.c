...
static int apple_bce_probe(struct pci_dev *dev, const struct pci_device_id *id)
{
    ...
    pr_info("apple-bce: pci-enable\n");
    if (pci_enable_device(dev))
        return -ENODEV;
    pr_info("apple-bce: pci-request-region %p\n", (void*)dev);
    if (pci_request_regions(dev, "apple-bce")) {
        status = -ENODEV;
        goto fail;
    }
    pr_info("apple-bce: pci-set-master %p\n", (void*)dev);
    pci_set_master(dev);
    pr_info("apple-bce: alloc-i %p\n", (void*)dev);
    ...
    bce->pci = dev;
    pr_info("apple-bce: set-drv-data\n");
    pci_set_drvdata(dev, bce);

    bce->devt = bce_chrdev;
    pr_info("apple-bce: device-create\n");
    ...
    pr_info("apple-bce: mbox init\n");
    bce_mailbox_init(&bce->mbox, bce->reg_mem_mb);
    pr_info("apple-bce: timestamp-init\n");
    bce_timestamp_init(&bce->timestamp, bce->reg_mem_mb);

    pr_info("apple-bce: spinlock\n");
    spin_lock_init(&bce->queues_lock);
    pr_info("apple-bce: ida_init\n");
    ida_init(&bce->queue_ida);

    pr_info("apple-bce: pci_request_irq\n");
    ...
    pr_info("apple-bce: dma-set-mask\n");
